...
async function ensureArtwork(en) {
  ...
  const file = path.join(os.tmpdir(), `ygo-art-${en.id}.${ext}`);
  await fs.writeFile(file, Buffer.from(await r.arrayBuffer()));
  artworkCache.set(en.id, file);
  return file;
}
...
function monsterType(de, en) {
  const race = de.race || en.race || 'Monster';
  const t = en.type || '';
  let kind = 'EFFEKT';
  ...
  return `${String(race).toUpperCase()} / ${kind}`;
}

function rendererData(card, en, de, artworkPath) {
  ...
  return {
    ...
    pendulumScale: 0,
    pendulumDescription: '',
    monsterType: type === 'monster' ? monsterType(de, en) : '',
    atkBar: true,
...
async function renderCard(card) {
  ...
  const art = await ensureArtwork(en);
  ...
  try {
    ...
    if (!result || result.error) throw new Error(result?.error?.message || 'Renderer-Export fehlgeschlagen');
...
async function makeThreeCardPdf(mode='render') {
  ...
  const page = pdf.addPage([420*MM, 297*MM]); // A3 landscape
  const cw = 59*MM, ch = 86*MM;
  const gap = 5*MM;
...
function shell(title, body) {
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#0e1116"><title>${esc(title)}</title><style>
  :root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#0e1116;color:#f4f6f8;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Text",Arial,sans-serif}.wrap{max-width:720px;margin:auto;padding:calc(22px + env(safe-area-inset-top)) 16px calc(30px + env(safe-area-inset-bottom))}h1{font-size:31px;margin:6px 0 8px}.sub{color:#a7b0bf;line-height:1.5}.card{margin-top:18px;background:#171c24;border:1px solid #313a49;border-radius:18px;padding:16px}.btn{display:block;width:100%;text-align:center;text-decoration:none;padding:15px;border-radius:14px;margin:10px 0;background:#202733;border:1px solid #394353;color:#fff;font-weight:700}.primary{background:#e8c56a;color:#18130a;border:0}.ok{color:#63d69a}.small{font-size:12px;color:#9da8b8;line-height:1.45}</style></head><body><div class="wrap">${body}</div></body></html>`;
}

app.get('/', (req,res) => {
  res.type('html').send(shell('YGO Render Test', `
    <div class="sub">CLASSIC BATTLE BOX · SERVER-TEST</div>
    <h1>Dragon's Roar</h1>
    <div class="sub">Das iPhone ist nur noch die Oberfläche. Bilder, Renderer und PDF laufen auf diesem Server.</div>
    <div class="card">
      ...
      <div class="small">Teste zuerst nur die einzelne Karte. Wenn die korrekt aussieht, gehen wir erst an das komplette 40-Karten-Deck.</div>
    </div>
  `));
});

app.get('/health', (req,res) => res.json({ok:true,node:process.version,assets:ASSET_ROOT}));
...
app.get('/test-card', async (req,res) => {
  try {
    ...
    console.error(e);
    res.status(500).type('html').send(shell('Renderfehler', `<h1>Renderfehler</h1><div class="card"><div style="color:#ef7373">${esc(e.stack || e.message || e)}</div></div>`));
  }
});
...
app.get('/test-pdf', async (req,res) => {
  try {
    ...
    console.error(e);
    res.status(500).type('html').send(shell('PDF-Fehler', `<h1>PDF-Fehler</h1><div class="card"><div style="color:#ef7373">${esc(e.stack || e.message || e)}</div></div>`));
  }
});

app.listen(PORT, '0.0.0.0', () => {
  console.log(`YGO render test listening on ${PORT}`);
});
